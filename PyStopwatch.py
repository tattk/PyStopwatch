...
class TrayIcon(QSystemTrayIcon):
    def __init__(self, parent):
        ...
        icon_path = os.path.join(base_path, "../Resources/app_icon.icns")
        self.setIcon(QIcon(icon_path))

        self.setToolTip("ストップウォッチ")
        self.menu = QMenu()
        
        self.show_action = QAction("ウィンドウを表示", self,checkable=True)
        self.show_action.setChecked(True)
        self.show_action.toggled.connect(parent.toggle_window)
        
        self.exit_action = QAction("終了", self)
        self.exit_action.triggered.connect(QApplication.quit) #sys.exit -> QApplication.quit
