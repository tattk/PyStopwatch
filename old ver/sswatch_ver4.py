...
class StopwatchApp(QMainWindow):
    def __init__(self):
        ...
        self.tray_icon.show()
        self.menubar_always_visible = True  # デフォルトでメニューバーに表示

        
    def toggle_menubar(self, checked): #l21~24 メニューバーへ常に表示機能を追加
        self.menubar_always_visible = checked
        if self.menubar_always_visible:
            self.tray_icon.show()
        else:
            self.tray_icon.hide()
...
class TrayIcon(QSystemTrayIcon):
    def __init__(self, parent):
        ...
        self.show_menu_action = QAction("メニューバーに常に表示", self, checkable=True) #常に表示へ変更
